...
//         lower quality to reduce their size, while smaller images have higher quality to maintain
//         their details. */
//       let quality = file.size > 10000000 ? 80 : file.size < 1000000 ? 100 : 88;
...
//         format = info.format;
//       } else if (file.mimetype.toString() === "image/gif") {
//         let { data, info } = await sharp(buffer, { animated: true })
//           .resize(200)
//           .toBuffer({ resolveWithObject: true });
//         buffer = data;
//         format = info.format;
...
//       // update format and blurhash
//       await supabase
//         .from("content")
//         .update({ format, blurhash })
//         .eq("id", media.id);

//       if (error) return;

//       let { error: uploadError } = await axios.put(
//         `${process.env.Image_Upload_Url}/${media.id}.${format}`,
//         buffer,
//         {
//           headers: {
...
//           tmp.file(
//             { postfix: path.extname(originalFileName) },
//             async (err, finalFile, fd, cleanupCallbackNew) => {
//               if (err) {
//                 return;
//               }
...
//                   // update supabase with the guid
//                   await supabase
//                     .from("content")
//                     .update({ id: response.data.guid })
//                     .eq("id", media.id);
